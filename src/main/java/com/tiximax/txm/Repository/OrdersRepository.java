...
""")
        ...
        WITH
                goods_by_staff_route AS (
                    SELECT
                        o.route_id,
                        o.staff_id,
                        SUM(ol.total_web) AS total_goods
                    FROM order_links ol
                    LEFT JOIN warehouse w ON ol.warehouse_id = w.warehouse_id
                    LEFT JOIN purchases p ON ol.purchase_id = p.purchase_id
                    JOIN orders o ON o.order_id = COALESCE(w.order_id, p.order_id)
                    WHERE COALESCE(w.created_at, p.purchase_time) >= :start
                      AND COALESCE(w.created_at, p.purchase_time) < :end
                      AND ol.status NOT IN (
                          'CHO_MUA', 'DA_MUA', 'DA_HUY',
                          'MUA_SAU', 'DAU_GIA_THANH_CONG'
                      )
                    GROUP BY o.route_id, o.staff_id
                ),
                payment_weight AS (
                    SELECT
                        ...
                        CASE
                            WHEN COUNT(*) > 1
                             AND SUM(COALESCE(ps.collect_weight, 0)) < 1
                                THEN 1
                            ELSE SUM(COALESCE(ps.collect_weight, 0))
                        END AS weight
                    FROM partial_shipment ps
                    ...
                    WHERE ps.payment_id IS NOT NULL
                      AND ps.shipment_date >= :start
                      AND ps.shipment_date < :end
                      AND ps.status = 'CHO_GIAO'
                    GROUP BY ps.payment_id, o.staff_id, o.route_id
                ),
                total_weight_by_staff_route AS (
                    SELECT
                        staff_id,
                        route_id,
                        SUM(weight) AS total_net_weight
                    FROM payment_weight
                    GROUP BY staff_id, route_id
                ),
                summary_by_staff_route AS (
                    SELECT
                        COALESCE(g.staff_id, t.staff_id) AS staff_id,
                        COALESCE(g.route_id, t.route_id) AS route_id,
                        COALESCE(g.total_goods, 0)       AS total_goods,
                        COALESCE(t.total_net_weight, 0)  AS total_net_weight
                    FROM goods_by_staff_route g
                    FULL JOIN total_weight_by_staff_route t
                           ON g.staff_id = t.staff_id
                          AND g.route_id = t.route_id
                    WHERE
                        COALESCE(g.total_goods, 0) <> 0
                     OR COALESCE(t.total_net_weight, 0) <> 0
                )
                SELECT
                    r.name        AS routeName,
                    s.staff_code  AS staffCode,
                    a.name        AS staffName,
                    sbr.total_goods      AS totalGoods,
                    sbr.total_net_weight AS totalNetWeight
                FROM summary_by_staff_route sbr
                JOIN staff s   ON s.account_id = sbr.staff_id
                JOIN account a ON a.account_id = s.account_id
                JOIN route r   ON r.route_id   = sbr.route_id
                WHERE a.role IN ('STAFF_SALE', 'LEAD_SALE')
                ORDER BY
                    r.name,
                    sbr.total_net_weight DESC,
                    s.staff_code;
    """, nativeQuery = true)
    List<Object[]> aggregateStaffKPIByRoute(
            @Param("start") LocalDateTime start,
    ...
    @Query(value = """
        WITH
                ...
                FROM total_weight_by_staff_route t
                JOIN staff s   ON s.account_id = t.staff_id
                JOIN account a ON a.account_id = s.account_id
                JOIN route r   ON r.route_id   = t.route_id
                WHERE a.role IN ('STAFF_SALE', 'LEAD_SALE')
                  AND t.total_net_weight > 0
                ORDER BY
                    r.name,
                    t.total_net_weight DESC,
                    s.staff_code;
    """, nativeQuery = true)
    List<Object[]> getShippingWeight(
            @Param("staffId") Long staffId,
